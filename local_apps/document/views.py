...
class DocumentListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        reporting_manager = self.request.query_params.get('reporting_manager', None)
        document_category = self.request.query_params.get('document_category', None)
        if companies:
            queryset = queryset.filter(document_user__company__id=companies)
        if department:
            queryset = queryset.filter(document_user__department__id=department)
        if shared_with:
            queryset = queryset.filter(document_user__shared_with__id=shared_with)
        if reporting_manager:
            queryset = queryset.filter(document_user__reporting_manager__id=reporting_manager)
        if document_category:
            queryset = queryset.filter(document_category__id=document_category)
        return queryset
